...
나만의 북마크 프로젝트는 Next.js 환경으로 개발하였으며 사용자가 개인적으로 관리하고 공유할 수 있는 웹 기반 북마크 관리 웹페이지입니다. 고유한 북마크 코드를 통해 여러 디바이스에서 접근하거나 다른 사람과 공유할 수 있는 기능을 제공합니다.

- **개발기간:** 2025.05.02 ~ 2025.06.27
- **프로젝트 인원** 1명
- [프로젝트 코드](https://github.com/kwongyunghwan/link)

## 사용자 흐름도

메인 페이지 -> 북마크 코드 입력 OR 새 코드 생성 ->
개인 북마크 페이지(READ) -> 북마크 추가(CREATE)/수정(UPDATE)/삭제(DELETE) 작업 -> 완성된 북마크 컬렉션

메인 페이지에서 북마크 코드를 입력하거나 새로 생성하면 개인의 북마크 페이지로 이동하고 해당 페이지에서 북마크를 추가 또는 수정 작업 후 이용하도록 하는것입니다.

## 🛠 기술 스택
### Back-end

- **Language:** JavaScript
- **Skill:** Next.js API Routes, MongoDB

#### Back-end 파트 설명 (Back-end)

- Next.js API Routes를 활용한 API 서버 구축
- MongoDB를 사용해 북마크 데이터 저장 및 관리
- FormData 처리를 통한 이미지 파일 업로드 로직 개발

### Front-end

- **Language:** JavaScript
- **Skill:** React.js, Next.js, Redux Toolkit

#### Front-end 파트 설명 (Front-end)

- 북마크 입력 및 관리 UI 개발
- Redux Toolkit을 사용해 북마크 코드 상태 관리 구현
- Next.js 기반 페이지 라우팅 및 SSR 활용
- CSS Grid/Flexbox를 통한 반응형 모달 인터페이스 설계
